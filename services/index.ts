...
export async function getAccounts(): Promise<Account[]> {
  ...
  for (const account of data) {
    ...
    account.initial_balance = balanceAcccounts[0].start_ammount ?? 0;
    account.current_balance = transactions.reduce(
      (accumulator, currentValue) => accumulator + currentValue.amount,
      balanceAcccounts[0].start_ammount ?? 0
    );
    account.dataChart = transactions.reduce(
      (acc, curr) => {
        acc.data.push({
          date: new Date(curr.date),
          amount: acc.total + curr.amount,
        });
        acc.total += curr.amount;
        return acc;
      },
      {
        total: balanceAcccounts[0].start_ammount ?? 0,
        data: [] as any[],
      }
    ).data;
  }
